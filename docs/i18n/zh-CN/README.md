...
- 新增决策动作 `update_tp_sl`：AI 可以在持仓中调整止盈/止损；执行层会先取消旧 TP/SL，再按当前仓位规模重下。
- 币安 TP/SL 注入：AI 上下文带有每个持仓的实时止盈/止损（来自当前开放委托），决策更贴合真实保护单。
- JSON 输出规则收紧：如需表达近似，使用中文“约”置于数字前，禁止 `~`、`≈` 等符号，避免解析异常。
- 交易员定时启动：在交易员列表可设置 HH:MM 定时启动，列表显示“已预约 HH:MM”徽章，并支持“定时启动/取消定时”操作和对应 toast 反馈。
- 手动启动会自动清除已预约的定时启动，避免到点重复触发。
- AI 交易员回撤保护可配置：可开关、可调“最低盈利触发% / 峰值回撤%”，保存后运行中的交易员立即按新参数重启监控（无需手动重启）。
- 手动历史同步（仅币安）：历史仓位页的“同步历史（近24h）”按钮会强制拉取过去24小时的成交/仓位并回填，无需等待后台同步。
- 持仓时间修复：暂停后重启交易员时，会正确保留持仓起始时间（DB 查询统一使用 LONG/SHORT 大写 side）。
